...
async function createJob(platform: 'shopify' | 'salesforce' | 'csv') {
  const [job] = await db.insert(syncJobs).values({ sourcePlatform: platform, status: 'pending' }).returning()
  return job.id
}
...
describe('runSync — shopify', () => {
  ...
  it('resolves transaction.customerId by matching customer email', async () => {
    ...
    const [emma] = await db.select().from(customers).where(eq(customers.email, 'emma.watson@example.com'))
    const emmaTxns = await db.select().from(transactions).where(eq(transactions.customerId, emma.id))
  ...
  it('stores amounts as integer cents and lowercase currency', async () => {
    ...
    const [txn] = await db.select().from(transactions).where(eq(transactions.sourceTransactionId, 'shp_ord_001'))
...
describe('runSync — salesforce', () => {
  ...
  it('converts dollar amounts to integer cents', async () => {
    ...
    const [txn] = await db.select().from(transactions).where(eq(transactions.sourceTransactionId, 'sf_opp_001'))
  ...
  it('resolves customerId via ContactEmail match', async () => {
    ...
    const [noah] = await db.select().from(customers).where(eq(customers.email, 'noah.williams@acmecorp.com'))
    const noahTxns = await db.select().from(transactions).where(eq(transactions.customerId, noah.id))
...
describe('runSync — csv', () => {
  it('parses valid rows and skips invalid ones without failing the whole job', async () => {
    ...
    for (let i = 0; i < 19; i++) rows.push(`good${i}@example.com,19.99,usd,completed,2024-01-01,row${i}`)
  ...
  it('fails the job when CSV error rate exceeds 5%', async () => {
    ...
    for (let i = 0; i < 20; i++) {
      rows.push(i < 5 ? `not-an-email,10,2024-01-01,row${i}` : `good${i}@example.com,10,2024-01-01,row${i}`)
    }
    const csv = rows.join('\n')
