...
async function insertTxn(
  amount: number,
  platform: 'stripe' | 'shopify' = 'stripe',
  sourceId = `t-${Math.random()}`
) {
...
async function createRun(graph: PipelineGraph) {
  const [pipeline] = await db
    .insert(pipelines)
    .values({ name: 'test', graphJson: graph as unknown as Record<string, unknown> })
    .returning()
  const [run] = await db
    .insert(pipelineRuns)
...
describe('runPipeline', () => {
    ...
    const graph: PipelineGraph = {
      nodes: [
        { id: 'src', type: 'source', data: { platform: 'stripe' } },
        {
          id: 'flt',
          type: 'transform',
          data: { ruleType: 'filter', config: { expression: 'amount > 1000' } },
        },
    ...
    const graph: PipelineGraph = {
      nodes: [
        { id: 'src', type: 'source', data: { platform: 'stripe' } },
        {
          id: 'bad',
          type: 'transform',
          data: { ruleType: 'filter', config: { expression: 'amount >' } },
        },
      ],
  ...
  it('rejects a graph with an unconfigured transform node before inserting steps', async () => {
    const graph: PipelineGraph = {
      nodes: [
        { id: 'src', type: 'source', data: {} },
        { id: 'unconfigured', type: 'transform', data: {} },
      ],
      edges: [{ id: 'e1', source: 'src', target: 'unconfigured' }],
    }

    const runId = await createRun(graph)
    await expect(runPipeline(runId, graph)).rejects.toThrow(/not configured/)
    ...
    const graph: PipelineGraph = {
      nodes: [
        { id: 'src', type: 'source', data: { platform: 'stripe' } },
        {
          id: 'rn',
          type: 'transform',
          data: { ruleType: 'rename', config: { columnMap: { amount: 'total' } } },
        },
        {
          id: 'dd',
          type: 'transform',
          data: { ruleType: 'dedupe', config: { keyFields: ['total'] } },
        },
        { id: 'sink', type: 'sink', data: {} },
      ],
      edges: [
