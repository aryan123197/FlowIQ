...
async function insertTxn(amount: number, status: 'completed' | 'failed' = 'completed', daysAgo = 0) {
  const date = new Date()
  date.setDate(date.getDate() - daysAgo)
  await db.insert(transactions).values({
    amount, currency: 'usd', status, sourcePlatform: 'stripe',
    sourceTransactionId: `t-${Math.random()}`, transactionDate: date,
  })
}
...
describe('evaluateAlerts', () => {
  ...
  it('creates a trigger when a rule breaches its threshold', async () => {
    ...
    const [rule] = await db.insert(alertRules).values({
      name: 'low revenue', metric: 'revenue_total', threshold: '99999',
      comparisonOperator: 'lt', timeWindow: '24h', actionType: 'slack',
  ...
  it('resolves a trigger once the metric no longer breaches', async () => {
    ...
    let unresolved = await db.select().from(alertTriggers)
    ...
    unresolved = await db.select().from(alertTriggers)
      .where(and(eq(alertTriggers.ruleId, rule.id), isNull(alertTriggers.resolvedAt)))
  ...
  it('skips disabled rules', async () => {
    await insertTxn(100)
    const [rule] = await db.insert(alertRules).values({
      name: 'disabled rule', metric: 'revenue_total', threshold: '99999',
      comparisonOperator: 'lt', timeWindow: '24h', actionType: 'slack',
      actionConfig: { webhookUrl: 'https://example.com/hook' }, enabled: false,
    }).returning()
